...
(function(root){
    ...
    function separateContours(path,stage){
        ...
        var boxes=parts.map(function(part){var node=document.createElementNS(NS,'path');node.setAttribute('d',part);stage.appendChild(node);var b=node.getBBox();node.remove();return {x:b.x,y:b.y,right:b.x+b.width,bottom:b.y+b.height};});
    ...
    async function exportObjects(source,onProgress){
        ...
        if(original.querySelector('image,foreignObject'))throw new Error('사진이 포함되어 있습니다. 먼저 SVG용 도형으로 변환하세요.');
        ...
        var count=0,visited=0;
        async function visit(node,paint,transforms){
            var tag=node.localName;if(['title','desc','metadata'].includes(tag))return;
            if(!['svg','g','path','rect','circle','ellipse','polygon','polyline','line'].includes(tag))throw new Error('지원하지 않는 SVG 요소: '+tag);
            ...
            var parts=tag==='path'?separateContours(node,stage):[null];
            ...
            for(var d of parts){
                ...
                for(var attr of node.attributes){if(!['id','transform','xmlns'].includes(attr.name))copy.setAttribute(attr.name,attr.value);}
                ...
                copy.setAttribute('id','object-'+String(++count).padStart(5,'0'));out.appendChild(copy);
            }
        ...
        return {svg:'<?xml version="1.0" encoding="UTF-8"?>\n'+new XMLSerializer().serializeToString(out),objects:count};
    }
